...
"""Provides the :py:class:`SquaredExponentialKernel` class that implements the
anisotropic SE kernel.
"""
    ...
    **kwargs
    ...
    def __init__(self, num_dim=1, **kwargs):
        param_names = [r'\sigma_f'] + [
            'l_{:d}'.format(i + 1,) for i in range(0, num_dim)
        ]
    ...
    def __call__(self, Xi, Xj, ni, nj, hyper_deriv=None, symmetric=False):
        ...
        if not only_first_order:
            ...
            if hyper_deriv is not None and hyper_deriv > 0:
                t = (tau[:, hyper_deriv - 1]) ** 2.0 / \
                    (self.params[hyper_deriv]) ** 3.0
                mask = n_combined[:, hyper_deriv - 1] > 0
                t[mask] -= n_combined[mask, hyper_deriv - 1] / \
        ...
        if hyper_deriv is None:
            ...
            if only_first_order:
                return (tau[:, hyper_deriv - 1]) ** 2.0 / \
                    (self.params[hyper_deriv]) ** 3.0 * k
            else:

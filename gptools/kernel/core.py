...
"""Core kernel classes: contains the base :py:class:`Kernel` class and helper
subclasses.
"""
...
class Kernel(object):
    ...
    num_dim : positive int
        ...
        the covariance kernel with. Default is 1.
    ...
    param_bounds : list of 2-tuples (`num_params`,), optional
        ...
        for each hyperparameter. Note that this is overridden by the
        `hyperprior` keyword, if present.
    ...
    enforce_bounds : bool
        If True, do not allow hyperparameters to be set outside of their
        bounds.
    params : :py:class:`Array` of float, (`num_params`,)
        Array of parameters.
    fixed_params : :py:class:`Array` of bool, (`num_params`,)
        Array of booleans indicated which parameters in :py:attr:`params` are
        fixed.
    ...
    param_bounds : :py:class:`~gptools.utils.CombinedBounds`
        The bounds on the hyperparameters. Actually a getter method with a
        property decorator.
    num_free_params : int
        Number of free hyperparameters. Actually a getter method with a
        property decorator.
    free_param_idxs : :py:class:`Array` of int, (`num_free_params`,)
        The indices of the free hyperparameters in :py:attr:`params`. Actually
        a getter method with a property decorator.
    free_params : :py:class:`~gptools.utils.MaskedBounds`
        Array of free parameters. Actually a getter method with a property
        decorator to permit modification in-place.
    free_param_bounds : :py:class:`~gptools.utils.MaskedBounds`
        The bounds on the free hyperparameters. Actually a getter method with a
        property decorator.
    free_param_names : :py:class:`~gptools.utils.MaskedBounds`
        List of the labels for the free hyperparameters. Actually a getter
        method with a property decorator.
    ...
    def __init__(self, num_dim=1, num_params=0, initial_params=None,
        ...
        if initial_params is None:
            ...
            if len(initial_params) != num_params:
                raise ValueError(
                    "Length of initial_params must be equal to num_params!"
                )
        ...
        if param_bounds is None and hyperprior is None:
            ...
            if (~self.fixed_params).any():
                warnings.warn(
                    ...
                    "creating the kernel, defaults may not be appropriate for "
                    "your data."
                )
            ...
            if param_bounds is not None and len(param_bounds) != num_params:
                raise ValueError(
                    "Length of param_bounds must be equal to num_params!"
                )
    ...
    def __call__(self, Xi, Xj, ni, nj, hyper_deriv=None, symmetric=False):
        """Evaluate the covariance between points `Xi` and `Xj` with derivative
        order `ni`, `nj`.
        ...
        raise NotImplementedError(
            "This is an abstract method -- please use one of the implementing "
            "subclasses!"
        )
    ...
    def set_hyperparams(self, new_params):
        ...
        if len(new_params) == len(self.free_params):
            if self.enforce_bounds:
                for idx, new_param, bound in zip(
                    range(0, len(new_params)),
                    new_params,
                    self.free_param_bounds
                ):
            ...
            raise ValueError(
                "Length of new_params must be {:d}!".format(
                    len(self.free_params)
                )
            )
    ...
    def free_param_bounds(self, value):
        # Need to use a loop since self.hyperprior.bounds is NOT guaranteed to
...
class BinaryKernel(Kernel):
    """Abstract class for binary operations on kernels (addition,
    multiplication, etc.).
    ...
    def __init__(self, k1, k2):
        if not isinstance(k1, Kernel) or not isinstance(k2, Kernel):
            raise TypeError(
                "Both arguments to BinaryKernel must be instances of type "
                "Kernel!"
            )
        if k1.num_dim != k2.num_dim:
            raise ValueError(
                "Only kernels having the same number of dimensions can be "
                "summed!"
            )
        self.k1 = k1
        self.k2 = k2
    ...
    def enforce_bounds(self):
        """Boolean indicating whether or not the kernel will explicitly enforce
        the bounds defined by the hyperprior.
        """
    ...
    def __call__(self, *args, **kwargs):
        ...
        if hd is not None:
            if hd < len(self.k1.params):
                ...
                return self.k2(
                    *args, hyper_deriv=hd - len(self.k1.params), **kwargs
                )
        else:
...
class ChainRuleKernel(Kernel):
    """Abstract class for the common methods in creating kernels that require
    ...
    - `_compute_y(self, tau, return_r2l2=False)`: Should return the
      "inner form" `y(tau)` to use with Faa di Bruno's formula. If
      `return_r2l2` is True, should also return the `r2l2` matrix from
      `self._compute_r2l2(tau)`.
    ...
    def _compute_dk_dtau(self, tau, n):
        r"""Evaluate :math:`dk/d\tau` at the specified locations with the
    ...
    def _compute_dk_dtau_on_partition(self, tau, p):
        """Evaluate the term inside the sum of Faa di Bruno's formula for the
        given partition.
...
class ArbitraryKernel(Kernel):
    ...
    num_params : int or None, optional
        ...
        the number of hyperparameters (but will fail if you used clever
        business with *args, etc.). Default is None (use inspection to find
        argument count).
    **kwargs
        All other keyword parameters are passed to
        :py:class:`~gptools.kernel.core.Kernel`.
    ...
    num_proc : non-negative int
        Number of processors to use in evaluating covariance derivatives. 0
        means serial.
    """
    def __init__(
        self, cov_func, num_dim=1, num_proc=0, num_params=None, **kwargs
    ):
        if num_proc is None:
    ...
    def _mask_cov_func(self, *args):
        ...
        *args : `num_dim` * 2 floats
            The individual elements of Xi and Xj to be passed to
            :py:attr:`cov_func`.
        """
        ...
        if self.num_dim == 1:
            ...
            return self.cov_func(
                args[:self.num_dim], args[self.num_dim:], *self.params
            )


class _ArbitraryKernelEval(object):
    """Helper class to support parallel evaluation of the
    :py:class:ArbitraryKernel:.
...
class MaskedKernel(Kernel):
    ...
    or has scaling/shifting.

    This can be used, for instance, to put a squared exponential kernel in one
    direction and a Matern kernel in the other.

    Overrides :py:meth:`__getattribute__` and :py:meth:`__setattr__` to make
    all setting/accessing go to the `base` kernel.
    ...
    scale : list or other array-like, optional
        1d list of scale factors to apply to the elements in `Xi`, `Xj`.
        Default is ones. Length must be equal to 2`base.num_dim`.
    """
    def __init__(self, base, total_dim=2, mask=[0], scale=None):
        if len(mask) != base.num_dim:
            raise ValueError(
                "Length of mask must be equal to the number of dimensions of "
                "the base kernel!"
            )
        if scale is None:
            ...
            raise ValueError(
                "Length of scale must be equal to twice the number of "
    ...
    def __getattribute__(self, name):
        ...
        if (
            ...
            name not in MASKEDKERNEL_RESERVED_NAMES
        ):
            try:

...
if __name__ == '__main__':
    ...
    # kernel.
    with open('demo/sample_data_core.pkl', 'rb') as f:
    ...
    # necessary.
    with open('demo/sample_data_edge.pkl', 'rb') as f:
    ...
    out = gp.predict(
        np.concatenate((X_star, X_star)),
        n=np.concatenate((np.zeros_like(X_star), np.ones_like(X_star))),
        full_output=True
    )
    ...
    y_synth = gp_noise.draw_sample(
        np.sort(core_data['X']), num_samp=10, noise=True
    )
    a_val.plot(np.sort(core_data['X']), y_synth, 'c.', alpha=0.5)
